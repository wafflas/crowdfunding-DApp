...
class CampaignList extends Component {
  ...
  loadCampaigns = async () => {
    try {
      const campaignsCount = await crowdfunding.methods.nextCampaignId().call(); 
      const campaigns = [];
      const userPledges = {};
      ...
      for (let i = 0; i < campaignsCount; i++) {
        ...
        const userPledge = await crowdfunding.methods.investments(i, currentAccount).call(); 

        const stateMap = ['Created', 'Cancelled', 'Fulfilled']; 
